...
__global__ void device_kernel_linear_t(const double *q, double *out, const double *vec, const double *in, const double QA_cost, const double cost, const int points, const int feature_range, const double add, const double gamma, const int id) {
    ...
    if (i >= j) { // if lower triangular matrix
        ...
        const int line = id_1d / (BLOCK_SIZE / 2);
        const int mem_num = id_1d % (BLOCK_SIZE / 2);
        const int transfer_line = id_1d / (BLOCK_SIZE / 4);
        const int tranfser_offset = id_1d % (BLOCK_SIZE / 4); 
        ...
        for (int feature_it = 8; feature_it < feature_range; feature_it = feature_it + 8) {
            const int off_plus = (feature_it & 8) * BLOCK_OFF * 2;
            const int off_minus = abs(off_plus - 16 * BLOCK_OFF);
            ...
            // Do 2 iterations
            for (int  mem_roll = 0; mem_roll < 2; ++mem_roll)
            {
                // load matrix_frag A
...
#pragma unroll
        ...
        bar[0].arrive_and_wait();
        const int off_last = ((feature_range - 8) & 8) * BLOCK_OFF * 2;
        ...
        if (id == 0) {
            ...
            if (threadIdx.y < BLOCK_SIZE / WARP_SIZE) {  // 96/32=3 --> Warp 0-2
                ...
                for (int store_it = 0; store_it < BLOCK_SIZE; ++store_it) {
                    const int index = (id_1d + store_it) % BLOCK_SIZE;
...
/*__global__ void device_kernel_linear_t(const double *q, double *out, const double *vec, const double *in, const double QA_cost, const double cost, const int points, const int feature_range, const double add, const double gamma, const int id) {
    ...
    if (i >= j) {
        ...
        for (int feature_it = 4; feature_it < feature_range; feature_it = feature_it + 4) {
            ...
            if (threadIdx.y >> 2 == 0) {  // threadIdx.y/4 --> warp 0-3
                // two warps per feature line for I
...
#pragma unroll
            for (int j_roll = 0; j_roll < ROLL_SIZE; ++j_roll) {  // first half of tensor operations - full i times half j
                double *const shmem_j_ptr = (double *) &Js[0] + 8 * ((j_roll + threadIdx.y) & 7) + off_minus; // j_roll
                nvcuda::wmma::load_matrix_sync(b_frag, shmem_j_ptr, BLOCK_OFF);
...
#pragma unroll
        for (int j_roll = 0; j_roll < ROLL_SIZE; ++j_roll) {  // first half of tensor operations - full i times half j
            double *const shmem_j_ptr = (double *) &Js[0] + 8 * ((j_roll + threadIdx.y) & 7) + off_last;
            nvcuda::wmma::load_matrix_sync(b_frag, shmem_j_ptr, BLOCK_OFF);
        ...
        // store solution part acc_frag C back in shared
        for (int frags = 0; frags < ROLL_SIZE; ++frags) {
            double *const shmem_m_ptr = (double *) &solution[0] + 8 * threadIdx.y * BLOCK_OFF + 8 * ((frags + threadIdx.y) & 7);
            nvcuda::wmma::store_matrix_sync(shmem_m_ptr, c_frag[frags], BLOCK_OFF, nvcuda::wmma::mem_row_major);
        }
...
__global__ void device_kernel_linear_tf(const float *q, float *out, const float *vec, const float *in, const float QA_cost, const float cost, const int points, const int feature_range, const float add, const float gamma, const int id) {
    ...
    if (i >= j) {
        const int id_1d = threadIdx.y * 32 + threadIdx.x;
        const int line = id_1d / (BLOCK_SIZE_F / 8);
        const int mem_num = id_1d & 15;
        ...
        for (int frags = 0; frags < ROLL_SIZE_F; ++frags) {
            // fill c_frags with zeros
            nvcuda::wmma::fill_fragment(c_frag[frags], 0.0f);
        }
        ...
        // loop over features
        for (int feature_it = 8; feature_it < feature_range; feature_it = feature_it + 8) {
            const int off_plus = (feature_it & 8) * BLOCK_OFF_F * 2;
            const int off_minus = abs(off_plus - 16 * BLOCK_OFF_F);
...
#pragma unroll
            for (int j_roll = 0; j_roll < ROLL_SIZE_F; ++j_roll) {
...
#pragma unroll
        ...
        // wait for new I-data to be loaded into shared memory
        bar[0].arrive_and_wait();
        const int off_last = ((feature_range - 8) & 8) * BLOCK_OFF_F * 2;
...
__global__ void device_kernel_poly_t(const double *q, double *out, const double *vec, const double *in, const double QA_cost, const double cost, const int points, const int feature_range, const double add, const int degree, const double gamma, const double coef0) {
    const int i = blockIdx.x * BLOCK_SIZE;
    const int j = blockIdx.y * BLOCK_SIZE;
    if (i >= j) { // if lower triangular matrix
        // helper variables for flow and copy
